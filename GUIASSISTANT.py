...
def main(text):
		...
		# if  True:
		# 	speak("Hello I am AARYA !", True)
		# 	speak("I'm your virtual assisstant", True)
		...
		if isContain(text, ['factorial','square root','root','log','value of','math','into','+','-','*',' x ','/','multiply','divided by','divide','binary','hexadecimal','octal','shift','power','to the power','sin ','cos ','tan ']):
			try:
		...
		if isContain(text, ['coin','dice','die']):
			if "toss" in text or "roll" in text or "flip" in text:
				...
				speak(result)
				return
		if isContain(text, ['search','results']) :
		...
		if result != "None": speak(result, True, True)
			...
			speak("Sorry, i didn't get you", True)
			speak("Try saying again", True)
